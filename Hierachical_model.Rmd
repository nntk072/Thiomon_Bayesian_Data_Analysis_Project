...
```{r}
# Define the model
hierarchical_formula_age <- bf(remission ~ 0 + hgb + hct + lymph_percent + neut_percent | age, 
                               family = bernoulli())
...
```{r}
# Define the model
hierarchical_formula_age <- bf(remission ~ 1 + hgb + hct + lymph_percent + neut_percent + 
                               (1 + 1 | age), 
                               family = bernoulli())
...
```{r}
# Define the model
hierarchical_formula_age_1 <- bf(remission ~ 0 + s(hgb) + s(hct) + s(lymph_percent) + s(neut_percent) + (1 + 1 | age), 
                               family = bernoulli())

# Fit the model
hierarchical_model_age_1 <- brm(
  formula = hierarchical_formula_age_1,
  data = train_data,
  iter = 2000,
  warmup = 1000,
  chains = 4,
...
# Calculate predicted probabilities on training data
train_predicted_probs_age_1 <- posterior_predict(hierarchical_model_age_1, newdata = train_data, draws = 1000, allow_new_levels = TRUE)
train_mean_probs_age_1 <- colMeans(train_predicted_probs_age_1)

# Find optimal threshold using ROC curve on training data
train_roc_curve_age_1 <- roc(train_data$remission, train_mean_probs_age_1)
optimal_threshold_age_1 <- coords(train_roc_curve_age_1, "best", ret = "threshold")

# Predict on test data
predicted_probs_age_1 <- posterior_predict(hierarchical_model_age_1, newdata = test_data, draws = 1000, allow_new_levels = TRUE)
mean_probs_age_1 <- colMeans(predicted_probs_age_1)

# ROC Curve
roc_curve_age_1 <- roc(test_data$remission, mean_probs_age_1)
plot(roc_curve_age_1, main = "ROC Curve for Age Model")

# Confusion Matrix
predicted_classes_age_1 <- ifelse(mean_probs_age_1 >= optimal_threshold_age_1$threshold, 1, 0)
conf_matrix_age_1 <- confusionMatrix(as.factor(predicted_classes_age_1), as.factor(test_data$remission))
print(conf_matrix_age_1)

# Report model summary
summary(hierarchical_model_age_1)
# Check EPLD LOO
loo(hierarchical_model_age_1)

# Check if chains converge
# variables(hierarchical_model_age_1)
mcmc_trace(hierarchical_model_age_1, pars=c("bs_shgb_1", "bs_shct_1", "bs_slymph_percent_1", "bs_sneut_percent_1")) + 
  scale_colour_manual(values=c("#000000", "#E69F00", "#56B4E9", "#009E73"))

# Autocorrelation function
mcmc_acf(hierarchical_model_age_1, pars=c("bs_shgb_1", "bs_shct_1", "bs_slymph_percent_1", "bs_sneut_percent_1"))

# Make posterior predictive checks
pp_check(hierarchical_model_age_1, type='dens_overlay', ndraws=200)
pp_check(hierarchical_model_age_1, type='loo_pit_qq', ndraws=1000)
pp_check(hierarchical_model_age_1, type='pit_ecdf', ndraws=1000)

# Test for prior sensitivity
model_draws_age_1 <- as_draws_df(hierarchical_model_age_1)

# Prior sensitivity is not there thanks to informative priors
powerscale_plot_dens(model_draws_age_1, fit=hierarchical_model_age_1, help_text=FALSE)

# Also the values look good!
powerscale_sensitivity(model_draws_age_1, fit=hierarchical_model_age_1) |> tt()

# Summarize the draws
model_draws_age_1 |> subset_draws(variable=c('lprior','lp__'), exclude=TRUE) |> summarise_draws() |> tt()

as_draws_df(model_draws_age_1) |> mcmc_areas(pars=c("bs_shgb_1", "bs_shct_1", "bs_slymph_percent_1", "bs_sneut_percent_1"))

# Extract random effects
ranef_hierarchical_age_1 <- ranef(hierarchical_model_age_1)

# Print random effects for age
print(ranef_hierarchical_age_1$age)
```
...
```{r}
ref_thiomon_formulae <- bf(remission ~ 0 + hgb + log_siirat + hct + (1 + 1 | deRitisRatio) , family = "bernoulli")
...
#Check if chains converge
mcmc_trace(thiomon_data_model, pars=c("b_hgb", "b_log_siirat", "b_hct")) + scale_colour_manual(values=c("#000000", "#E69F00", "#56B4E9", "#009E73"))
...
as_draws_df(model_draws) |> mcmc_areas(pars=c('b_hgb', 'b_log_siirat', 'b_hct'))
```
...
# Calculate predicted probabilities on training data
train_predicted_probs_age <- posterior_predict(thiomon_data_model, newdata = train_data, draws = 4000, allow_new_levels = TRUE)
train_mean_probs_age <- colMeans(train_predicted_probs_age)
...
# Predict on test data
predicted_probs_age <- posterior_predict(thiomon_data_model, newdata = test_data, draws = 1000, allow_new_levels = TRUE)
mean_probs_age <- colMeans(predicted_probs_age)
...
```{r}
ref_thiomon_formulae <- bf(remission ~ 0 + (hgb | hct) + log_siirat  + deRitisRatio + (1|age) , family = "bernoulli")
thiomon_priors_default_priors <- get_prior(ref_thiomon_formulae, data = train_data)
thiomon_set_priors_informative <- c(
  prior(
    normal(7,1),
