...
#temp_df <- thiomon[, c("hgb", "lymph_percent", "hct", 
                       #"neut_percent", "plt", "alb", "ast")]
temp_df <- thiomon
...
```{r}
#rstudent_t(n=20000, df=3, mu=15, sigma=3.0)
#rbeta
data.frame(theta = rbeta(20000,30,30,ncp=10)) |>
  mcmc_hist(binwidth = .1) +
  labs(title='Default brms student_t(3, 0, 2.5) for Intercept')
```

```{r}
#thiomon_formulae <- bf(remission ~ 0 + hgb + hct + alb, family = "bernoulli")
thiomon_formulae <- bf(remission ~ 0 + hgb + lymph_percent + hct + neut_percent + plt + alb + ast + alk + mono_percent + rdw + mch + wbc + age + mcv + eos_percent + alt + mpv + baso_percent + gluc + mchc + tbil + cal + prot + un + pot + chlor + co2 + sod + creat, family = "bernoulli")
...
```{r}
thiomon_formulae_2 <- bf(remission ~ 0 + hgb + mch + lymph_percent + neut_percent + eos_percent + mono_percent + creat + mpv + alb + baso_percent, family = "bernoulli")

thiomon_priors_2 <- get_prior(thiomon_formulae_2, data = sampled_df)

thiomon_data_model_2 <- brm(
    formula = thiomon_formulae_2,
    prior = thiomon_priors_2,
    data = sampled_df,
    save_pars = save_pars(all = TRUE),
    iter = 2000,
    warmup = 1000,
...
```{r}
thiomon_formulae_4 <- bf(remission ~ 0 + hgb, family = "bernoulli")

thiomon_priors_4 <- get_prior(thiomon_formulae_4, data = sampled_df)

(thiomon_set_priors <- c(
  prior(
    normal(10,5),
    class = "b",
    coef = "hgb"
...
thiomon_data_model_4 <- brm(
    formula = thiomon_formulae_4,
    prior = thiomon_set_priors,
    data = sampled_df,
    save_pars = save_pars(all = TRUE),
    iter = 2000,
    warmup = 1000,
...
thiomon_formulae_3 <- bf(remission ~ 0 + hgb + log_siirat + hct + deRitisRatio, family = "bernoulli")

thiomon_priors_3 <- get_prior(thiomon_formulae_3, data = sampled_df)

(thiomon_set_priors_3 <- c(
  prior(
    normal(10,5),
    class = "b",
    coef = "hgb"
...
thiomon_data_model_3 <- brm(
    formula = thiomon_formulae_3,
    prior = thiomon_set_priors_3,
    data = sampled_df,
    save_pars = save_pars(all = TRUE),
    iter = 2000,
    warmup = 1000,
...
```{r}
fit_dist <- fitdist(source_data$deRitisRatio, "weibull")
summary(fit_dist)
plot(fit_dist)
```
...
```{r}
loo_compare(loo(thiomon_data_model), loo(thiomon_data_model_2), loo(thiomon_data_model_3)) |>
  as.data.frame() |>
...
predicted_probs <- posterior_predict(thiomon_data_model_3, 
                                     newdata = test_df, draws = 1000)
...
```{r}
draws <- as_draws_df(thiomon_data_model_4)

powerscale_plot_dens(draws, fit=thiomon_data_model_3,
                     help_text=FALSE)

powerscale_sensitivity(draws, fit=thiomon_data_model, variable="b_lymph_percent") |>
  tt()

mcmc_trace(thiomon_data_model, pars=c("b_hgb")) + scale_colour_manual(values=c("#000000", "#E69F00", "#56B4E9", "#009E73"))
