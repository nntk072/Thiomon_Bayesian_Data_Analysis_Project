...
thiomon_formulae <- bf(remission ~ 0 + hgb + hct + lymph_percent + neut_percent, family = "bernoulli")
thiomon_priors <- get_prior(thiomon_formulae, data = sampled_df)
...
```{r}
# Define the formula
thiomon_formulae <- bf(remission | trials(1) ~ 1 + hgb + hct + lymph_percent + neut_percent + plt + alb + ast + age + mcv_wbc_rat + sii_rat, family = "binomial")

# Get priors
thiomon_priors <- get_prior(hierarchical_formula, data = store_data)
