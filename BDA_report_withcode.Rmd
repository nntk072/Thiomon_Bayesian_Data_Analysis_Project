...
Posterior predictive checks seem fine as well. Bars in the plot seems to fit well.
...
The Pareto k-estimates indicate there are no observations which have k \> 0.7. Posterior predictive checks seem fine as well. Bars plots seems to fit well.
...
predicted_probs <- posterior_predict(thiomon_data_model_hier_2, 
                                     newdata = test_data, draws = 4000, allow_new_levels = TRUE)
...
Besides the implementation of Bayesian model, we would like to introduce the machine learning aspect to the model as well for comparison between Bayesian approach and simple machine learning approach, while finding and evaluating the correlation between model parameters for saving computational resources and suggesting the observation that we can try in Bayesian model as well. Support vector machine (SVM) model and General Linear model (GLM) will be described with the following parameters based on the abbreviation of dataset description /cite something here. In general, the model will use all of the parameters of the dataset, without any modification, for the purpose of comparison only
...
After building the model, dut to the objective of the model is classifation, we can use classification evaluation metrics aspect for evaluation the model result. Two suggestion that we have implemented: using confusion matrix with evaluation metrics mathematical calculation, and using ROC curves with AUC values for different model comparison, which can be elaborated in session 4.1. and 4.2.

## 4.1. Confusion matrix and Evaluation metrics results

Confusion matrix is a common method for binary classification task, where the prediction will be built in the augmented matrix, describing the actual status of the model prediction. Here are the detailed results for each model:
...
The accuracy of the pooled model is close to random guessing, as indicated by the balanced accuracy of 49.98%.
...
The separate model performs better than the pooled model, with an accuracy of 60.32%. The balanced accuracy of 60.64% indicates a moderate performance. The model has a higher sensitivity, making it better at identifying true positives.

### 4.1.3. Hierarchical model
...
This hierarchical model has an accuracy of 59.07% and a balanced accuracy of 59.44%. The sensitivity is higher than the specificity, indicating that the model is better at identifying true positives.

### 4.1.4. Hierarchical model with pooled
...
Hierarchical model 2 performs slightly better than hierarchical model 1, with an accuracy of 59.78% and a balanced accuracy of 60.14%. The sensitivity is higher, indicating better identification of true positives.
...
The GLM model outperforms all other models with an accuracy of 67.32% The model has a good balance between sensitivity and specificity, making it effective at identifying both true positives and true negatives.
...
According to the evaluation result, we could see that almost all the models are having the AUC difference between training and testing data maximum 7%, which can be an acceptable value. Moreover, the Bayesian approach cannot be better than the GLM approach. One of the proposal for this is because of having less occupied parameters in the model. We can base on that to improve our model as well.

### 4.3. Discussion.

According to the evaluation results, the GLM model outperforms the Bayesian models in terms of accuracy and AUC. Moreover, asexpected from the bayesian analysis, the difference between Separate and Hierarchical here is not clearly. The Bayesian models show moderate performance but do not surpass the GLM model.

# 5. Discussion of Issues and Potential Improvements

The main issue observed is the moderate performance of the Bayesian models compared to the GLM model. To improve the accuracy of the Bayesian models, we can consider increasing the number of variables and normalizing the parameters into different categories and medical terms. This can help improve the model's accuracy and predictive power.

## 5.1. Improving the number of training sample

An example of applying 4500 variables into the separate model with more

An example for that is using different parameters, when we decide the model into different parameters, which has been normalized as follow
...
All of the bayesian check methods have been produced and did not provide any problems compared to the previous model. Therefore, we could come to evaluation session
...
Base on observation, the actual accuracy of the model here is larger, but not significant. However, the generalization of the model is better due to the decrease in the sensitivity and increase and specificity. This is a good sign for the model as well.
...
The model here can be computed with more parameters, with the purpose of producing a better result. However, using more paramters can also lead to increase the model complexity, which may not be approriate in the task's project. Therefore, an example of changing the separate model formula with adding 4 more parameters hct, wbc, mpv, mchc into the model and we can observe the result.
...
## 5.3. ROC curves/AUC values comparison
...
train_roc_data <- rbind(
  data.frame(roc_separate_train, model = paste("Separate Train (AUC =", round(auc_separate_train, 4), ")")),
  data.frame(roc_separate_improvement_train, model = paste("Separate model + more training samples (AUC =", round(auc_separate_improvement_train, 4), ")")),
  data.frame(roc_separate_improvement_1_train, model = paste("Separate model + more variables (AUC =", round(auc_separate_improvement_1_train, 4), ")"))
)
test_roc_data <- rbind(
  data.frame(roc_separate_test, model = paste("Separate Test (AUC =", round(auc_separate_test, 4), ")")),
    data.frame(roc_separate_improvement_test, model = paste("Separate model + more training samples (AUC =", round(auc_separate_improvement_test, 4), ")")),
    data.frame(roc_separate_improvement_1_test, model = paste("Separate model + more variables (AUC =", round(auc_separate_improvement_1_test, 4), ")"))
)
# Plot all ROC curves in one plot
ggplot(train_roc_data, aes(x = fpr, y = tpr, color = model)) +
  geom_line() +
...
## 5.2. Potential Improving

Moreover, besides these trials that have been implemented, there can be room for improvement. An example is normalizing the parameters into medical terms for building model with efficient parameters only. Furthermore, we have not tried to used with spline approach in the model, or with multi-parameters in hierarchical models. They are worth for considering when we approach the problems and can be the necessary steps for improvement.

One improvement which we consider is to use R2D2 method for the priors and using the full set of covariates for the modelling.
