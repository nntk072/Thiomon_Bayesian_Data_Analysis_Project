...
Confusion matrix is a common method for binary classification task, where the prediction will be built in the augmented matrix, describing the actual status of the model prediction. There will....

Besides that, we can see different evaluation metrics mathematical calculation as the consequence of confusion matrix as well...
...
pred_classes_svm_test <- ifelse(pred_probs_svm_test > optimal_threshold_svm$threshold, 1, 0)
conf_matrix_svm_test <- confusionMatrix(as.factor(pred_classes_svm_test), as.factor(test_data$remission))
...
According to the evaluation result, we could see that almost all the models are having the AUC difference between training and testing data maximum 7%, which can be an acceptable value. Moreover, the Bayesian approach cannot be better than the GLM approach, due to the reason of having less occupied parameters in the model. We can base on that to improve our model as well.

However, with the AUC values representing in SVM model, we can observe the affect of using multiple variables without filtering can also lead to the overfitting problem, which is the problem of choosing parameters without carefully decided. Therefore, the number and the choice of paramters must be experimental carefully

# 5. Discussion of Issues and Potential Improvements

According to the result, we do not have any problems with the posterior check or the model evaluation status. The only issues that we need to improve is the improvement in model accuracy. Increasing the number of parameters and normalizing the parameters into different categories and medical terms can help improve the model accuracy.
...
After evaluating various models on the Thiomon dataset, we can conclude that the effects of the variables regarding remission are considerable. The Bayesian approach for this remission classification task is not compatible, especially with the current model. However,  However, the Bayesian approach is computationally complex, where increasing the number of parameters will lead to more time consumption during the fitting process. Therefore, we can expect more accuracy improvement when we build a larger model.

Discussing the pooled model with regression coefficients only, we can see that it follows a random guess, leading to AUC values approximating 0.54, which is the accuracy of the dataset distribution as well.

With the different Bayesian models, the parameters we can obtain for each model are:

Separate model
```
remission ~ (-2.50 +- 0.51) * hgbrat1  + (-1.37 +- 0.12)* hgbrat2  + (-1.14, SE = 0.08) * hgbrat3  + (-0.24, SE = 0.08) * hgbrat4  + (0.17, SE = 0.20) *hgbrat5  + 0.05 * lymph_percent 
```

Hierarchical model with global coefficient:
```
remission ~ Intercept (-1.5, SE = 0.2) + lymph_percent (0.05, SE = 0.00) + (0.036 +- 0.008) sd_hgbrat__Intercept 
```
hierarchical model without global coefficient:
```
remission ~ lymph_percent (0.05, SE = 0.00) + sd_hgbrat__Intercept (0.036, SE = 0.008)

```

Compared to these models, the hierarchical models, particularly the one with a global coefficient, provided a good balance between capturing global and group-level variations, leading to the best overall performance. The SVM and GLM models also showed promising results. However, this is not the main concept of the project, which can be included for introducing the limitation of Bayesian model compared to them. Moreover, SVM and GLM also describe us the potential of using more parameters in Bayesian approach can lead to better result, which can be done with more computational resources in the potential development.

Lastly, future work could focus on further exploring additional features to improve predictive performance.
